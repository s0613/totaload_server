...
import com.isoplatform.api.auth.dto.WithdrawResponse;
...
import com.isoplatform.api.auth.service.AppleTokenService;
...
public class AuthController {
     ...
     *
     * @param request login request containing username or email and password
    ...
    public ResponseEntity<AuthResponse> getCurrentUser(Authentication authentication) {
        if (authentication == null || !authentication.isAuthenticated()) {
            log.warn("Unauthorized /me request");
    ...
    /**
     * Mobile Google Sign-In - Google ID token to JWT
     * Used by mobile apps that exchange tokens directly with Google (via flutter_appauth)
     */
    @PostMapping("/mobile/google")
    public ResponseEntity<MobileTokenResponse> mobileGoogleSignIn(
        ...
        try {
            ...
            log.info("Mobile Google sign-in request received, deviceInfo: {}", deviceInfo);

            // 1. Verify Google ID token and extract user info (mobile audience)
            GoogleTokenService.GoogleUserInfo googleUser = googleTokenService.verifyIdTokenMobile(idToken);

            // 2. Find or create user (using sub as providerId, NOT email)
            ...
            // 3. Update email if changed (email can change, sub cannot)
            ...
            // 5. Generate and store refresh token in DB (7 days)
            ...
            log.info("Mobile Google sign-in successful for user: {}", user.getEmail());
            ...
            log.error("Mobile Google sign-in failed: {}", e.getMessage());
    ...
    /**
     * Mobile Apple Sign-In - Apple Identity Token to JWT
     * Used by iOS apps that authenticate with Sign in with Apple
     *
     * @param request Apple sign-in request containing identity token
     * @return 200 OK with JWT tokens
     */
    @PostMapping("/mobile/apple")
    public ResponseEntity<MobileTokenResponse> mobileAppleSignIn(
            @Valid @RequestBody AppleSignInRequest request) {
        log.info("Mobile Apple sign-in request received");

        // 1. Verify Apple Identity Token and extract user info
        AppleTokenService.AppleUserInfo appleUser = appleTokenService.verifyIdToken(
                request.getIdentityToken());

        // 2. Find or create user (using sub as providerId)
        User user = userRepository.findByProviderAndProviderId("APPLE", appleUser.getSub())
                .orElseGet(() -> {
                    // 첫 로그인 시 이름과 이메일은 request에서 가져옴 (Apple은 첫 로그인에만 제공)
                    String email = appleUser.getEmail() != null ?
                            appleUser.getEmail() : request.getEmail();
                    String name = request.getName() != null ?
                            request.getName() : "Apple User";

                    User newUser = User.builder()
                            .email(email)
                            .provider("APPLE")
                            .providerId(appleUser.getSub())
                            .name(name)
                            .password("")  // OAuth users don't have password
                            .role(Role.USER)
                            .company("SELF")
        ...
        // 3. Update email if changed and provided
        String appleEmail = appleUser.getEmail();
        if (appleEmail != null && !appleEmail.equals(user.getEmail())) {
            user.setEmail(appleEmail);
            userRepository.save(user);
        }
        ...
        log.info("Mobile Apple sign-in successful for user: {}", user.getEmail());
    ...
    /**
     * Withdraw user account (회원 탈퇴)
     * Requires valid access token in Authorization header
     * For LOCAL users, password verification is required
     *
     * @param authentication Spring Security authentication (from JWT)
     * @param request withdraw request containing reason and password (for LOCAL users)
     * @return 200 OK with withdraw response
     */
    ...
    public ResponseEntity<WithdrawResponse> withdraw(
        ...
        String email = authentication.getName();
        log.info("Withdraw request for user: {}", email);

        User user = userRepository.findByEmail(email)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "User not found"));

        withdrawService.withdraw(user.getId(), request);

        return ResponseEntity.ok(WithdrawResponse.success());
    }
}
